...
class Stitch:
	def __init__(self, file):
		self.path = file
		f = open(self.path, 'r')
		filenames = [each.rstrip('\r\n') for each in  f.readlines()] # read each line to get filenames
		print (filenames)
		self.images = [cv2.resize(cv2.imread(each),(480, 320)) for each in filenames]# after read images, reset size to the same one(width, height)
		self.count = len(self.images)
		self.prepare_lists()
		self.left_list, self.right_list, self.centerImg = [], [], None
		self.matcher_obj = matchers()# call matchers as a subclass

	def prepare_lists(self):
		print ("Number of images : %d"%self.count)
		self.centerNum = int(self.count/2) 
		print ("Center index image : %d"%self.centerNum)
		self.centerImg = self.images[self.centerNum]# decide the center image
